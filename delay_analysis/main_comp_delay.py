...
command = "ngspice destination_first_comp.ckt"
...
for j in range(0,8): #j represents the iteration of input. since we have 8 inputs we have kept it as 0,8
    if j<4:
      ...
      #i is for output sequence
    
    for i in range(0,3):
        if i == 0:
            s2 = "comparator_0/greater_than"
        elif i == 1:
            s2 = "comparator_0/equal_to"
        elif i == 2:
            s2 = "comparator_0/less_than"
        #fp1 is src file since you cant modify it you open it in read mode
        
        if(j<4):
            fp1 = open("first1_comp.ckt",'r') #for input sequence 1
        else:
            fp1 = open("first2_comp.ckt",'r') #for input sequecne 2
        #if you have any other input sequences you can add

        fp2 =open("destination_first_comp.ckt",'w') #destination file for running
